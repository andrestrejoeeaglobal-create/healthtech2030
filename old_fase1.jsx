...
export const Fase1_Identificacion = ({
    ...
    const starterMessage = {
        role: 'assistant',
        content: "Para comenzar con su historia cl├¡nica, validemos el resto de su identidad.\n\n┬┐Cu├íl es su **Apellido Paterno**?"
    };
            ...
            switch (step) {
                ...
                case 'intro_dob_day': {
                    ...
                    if (isNaN(day) || day < 1 || day > 31) {
                        responseMsg = "Por favor indique un d├¡a v├ílido (1-31).";
                    } else {
                        setTempDob(prev => ({ ...prev, day: day }));
                        responseMsg = "┬┐En qu├® **mes**? (Ej: 5 o Mayo)";
                ...
                case 'intro_dob_month': {
                    ...
                    if (!monthCode || monthCode < 1 || monthCode > 12) {
                        responseMsg = "Mes no v├ílido. Intente escribir el n├║mero (Ej: 5) o el nombre completo.";
                    } else {
                        setTempDob(prev => ({ ...prev, month: monthCode }));
                        responseMsg = "┬┐De qu├® **a├▒o**? (Ej: 1990)";
                ...
                case 'intro_dob_year': {
                    ...
                    if (isNaN(year) || year < 1920 || year > currentYear) {
                        responseMsg = "A├▒o no v├ílido. Escriba 4 d├¡gitos (Ej: 1990).";
                        break;
                    }
                    ...
                    responseMsg = `Registrado (${age} a├▒os - ${lifeStage}).\n\n┬┐Cu├íl es su **sexo biol├│gico** al nacer?`;
                    options = [
                ...
                case 'intro_sex': {
                    ...
                    if (sex !== "Masculino" && sex !== "Femenino") {
                        responseMsg = "Por favor seleccione una opci├│n v├ílida (Masculino o Femenino).";
                        options = [
                            { label: "Masculino", value: "Masculino" },
                            { label: "Femenino", value: "Femenino" }
                        ];
                        ...
                        // BYPASS LOGIC (L├│gica Kin├®tica)
                        const currentAge = identityLock.patientInfo?.age || 20; // fallback safety
                        if (currentAge < 18) {
                            // Si es menor de 18, setear soltero y omitir ocupaci├│n
                            updateSocioculturalProfile({ civilStatus: "Soltero", occupation: "Estudiante/Menor" });
                            responseMsg = "Debido a su edad, he omitido los datos laborales. ┬┐Profesa alguna **religi├│n**? (Importante para determinar dietas, Ej: Morm├│n, Cat├│lico. Escriba 'Ninguna' si no aplica)";
                            nextStep = 'intro_religion';
                        } else {
                            responseMsg = "┬┐Cu├íl es su **estado civil** actual?";
                            options = [
                                { label: "Soltero", value: "Soltero" },
                                { label: "Casado", value: "Casado" },
                                { label: "Uni├│n Libre", value: "Uni├│n Libre" },
                ...
                case 'intro_civil_status':
                    ...
                    responseMsg = "┬┐A qu├® se dedica actualmente? (Ej: Docente, Estudiante, Ingeniero)";
                    nextStep = 'intro_job';
                    break;

                case 'intro_job':
                    updateSocioculturalProfile({ occupation: cleanText });
                    responseMsg = "┬┐Profesa alguna **religi├│n**? (Importante para determinaciones diet├®ticas como restricciones de carne/suplementos. Si no tiene, escriba 'Ninguna')";
                    nextStep = 'intro_religion';
                    break;
                ...
                default:
                    responseMsg = "Esta secci├│n ha concluido.";
                    break;
            }
