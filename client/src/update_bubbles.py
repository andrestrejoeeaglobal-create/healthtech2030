...
files = [
    'Fase4_AntecedentesFamiliares.jsx',
    'Fase5_EstiloVida.jsx',
...
new_bubble = '''<div className={`p-4 rounded-2xl max-w-[85%] shadow-sm ${msg.role === 'assistant'
                                ? msg.isBio
...
pattern = re.compile(r'<div\s+className=\{`p-4 rounded-2xl max-w-\[85%\] shadow-sm \$\{msg\.role === [\'"]user[\'"]\s*\?\s*[\'"]bg-indigo-600 text-white rounded-tr-none[\'"]\s*:\s*[\'"]bg-white border border-slate-100 text-slate-700 rounded-tl-none[\'"]\s*\}`\}>', re.MULTILINE)
