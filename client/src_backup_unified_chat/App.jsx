...
function App() {
  ...
  useEffect(() => {
    ...
    } else if (currentPhase !== 'PHASE_0_AUTH' && currentPhase !== 'PHASE_0_IDENTITY_CHECK' && currentPhase !== 'PHASE_0_PRIVACY') {
      // ✅ FIX: Sincronización automática de Dashboard (Espejo Clínico) si se recarga la página o se restaura sesión desde localStorage
            ...
            {interviewStep === 'finished' ? (
              ...
              <Fase3_MotivoConsulta
                ...
                onPhaseComplete={(motiveData) => {
                  ...
                  // 1. Marcar el Triaje como completado en el estado global (Destraba el acordeón visual)
                  setPatientData(prev => ({
                    ...prev,
                    clinical_triage: { ...prev.clinical_triage, triage_completed: true }
                  }));

                  // 2. Mover la Fase del Motor del Chat
                  setCurrentPhase('PHASE_4_FAMILY_HISTORY');
                  
                  // 3. Forzar el cambio del Acordeón del Panel Izquierdo
                  ...
                  {interviewStep === 'intro_curp' && (
                    ...
                    >
                      <button
                        ...
                        className="text-xs font-bold bg-white text-slate-700 px-3 py-2 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors flex items-center gap-2"
                      >
                      ...
                      <div className="relative flex items-center gap-2 bg-white border border-slate-200 rounded-full px-2 py-2 shadow-sm focus-within:ring-4 focus-within:ring-blue-50 focus-within:border-blue-400 transition-all w-full">
                        ...
                        {messages.length === 0 || messages[messages.length - 1].inputType !== 'strict_select' ? (
                          <button
                            ...
                            className="bg-[#1C75BC] text-white w-10 h-10 flex items-center justify-center rounded-full hover:bg-[#155a8a] transition-transform active:scale-95 shadow-md flex-shrink-0"
                          >
