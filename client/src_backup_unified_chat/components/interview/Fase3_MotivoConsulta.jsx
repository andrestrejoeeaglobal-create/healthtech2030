...
const getFollowUpPrompt = (route, label) => {
    // Grupo B: Optimización (Sin enfoque en patología/dolor)
    if (route === 'GOAL_WEIGHT_LOSS') return `Entendido. Hemos activado la ruta de **${label}**.

---

¿Cuáles son sus objetivos principales de composición corporal?`;

    if (route === 'GOAL_MUSCLE') return `Entendido. Hemos activado la ruta de **${label}**.

---

¿Cuál es su disciplina deportiva principal o nivel de entrenamiento actual?`;

    if (route === 'GOAL_LONGEVITY') return `Entendido. Hemos activado la ruta de **${label}**.

---

¿Qué áreas de su rendimiento físico o cognitivo busca optimizar en este protocolo?`;
    
    // Grupo C: Transición de Vida (Sin enfoque en patología/dolor)
    if (route === 'GOAL_PREGNANCY') return `Entendido. Hemos activado la ruta de **${label}**.

---

¿En qué etapa gestacional o de lactancia se encuentra actualmente?`;

    if (route === 'GOAL_GERIATRICS') return `Entendido. Hemos activado la ruta de **${label}**.

---

¿Cuáles son los retos principales en su alimentación diaria que busca mejorar con este abordaje?`;

    if (route === 'GOAL_MENOPAUSE') return `Entendido. Hemos activado la ruta de **${label}**.

---

¿Cuáles son los síntomas o cambios principales que ha experimentado recientemente?`;

    if (route === 'GOAL_PEDIATRICS') return `Entendido. Hemos activado la ruta de **${label}**.

---

¿Cuál es el principal reto de desarrollo o crecimiento que buscan abordar en esta consulta?`;

    // Grupo A: Clínico/Patológico (Permite preguntar por síntomas)
    return `Entendido. Hemos activado la ruta clínica de **${label}**.
...
const Fase3_MotivoConsulta = ({ messages, setMessages, onPhaseComplete, patientData, setPatientData }) => {
    ...
    const { patientAge: age, patientSex: gender, patientName, isMinor } = usePatientLinguistics(patientData);
    ...
    const getBinaryGateReviewMsg = () => {
        const cleanName = patientName !== "NOM" ? patientName : "paciente";
        const firstName = cleanName.split(' ')[0];
        
        let displayName = "Estimado paciente";
        if (isMinor) {
            const tutorRelation = patientData?.emergencia?.parentezco;
            const tutorName = (patientData?.emergencia?.nombre) ? patientData.emergencia.nombre.split(' ')[0] : "Tutor";
            
            if (tutorRelation === "Madre") {
                displayName = `Estimada ${tutorName}`;
            } else if (tutorRelation === "Padre") {
                displayName = `Estimado ${tutorName}`;
            } else {
                displayName = `Estimado(a) ${tutorName}`;
            }
        } else if (firstName !== "paciente") {
            displayName = firstName;
        }

        return {
            role: "assistant",
            content: `📍 **Bloque de Triage Clínico Preliminar Sellado**

*Nota Clínica:* La información capturada y el análisis paramétrico de T.I.L.O. 
formarán la base integral de su abordaje clínico, en estricto apego a los 
lineamientos de confidencialidad de la NOM-004.

---

**${displayName}**, por favor valide que esta evaluación clínica es precisa y representa fielmente el motivo de consulta:`,
            options: [
                { label: "✅ SÍ, CONFIRMAR EVALUACIÓN", value: "CONFIRM_DATA" },
    ...
    const [step, setStep] = useState(() => {
        ...
        if (context.goal) {
            if (context.secondary_symptoms || context.sintomas) {
                ...
                // Reglas de Exclusión de Mapa Corporal
                const skipBodyMap = [
                    'GOAL_PREGNANCY', 
                    'GOAL_WEIGHT_LOSS', 
                    'GOAL_MUSCLE', 
                ...
                if (skipBodyMap.includes(context.goal)) {
                    // Si ya contestó los síntomas secundarios y no requiere mapa corporal
                    return context.ai_analysis?.gem_reasoning ? 'clinica_triage_review' : 'clinica_triage_symptoms';
                }
                ...
                case 'clinica_triage_start': {
                    ...
                    if (selectedOpt) {
                        ...
                        analyzeClinicalMotive(cleanText, telemetry, bodyMapZones).then(aiResult => {
                             ...
                             finalResponse += `\n\n---\n\n${getFollowUpPrompt(aiResult.primaryRoute, routeToSpanish(aiResult.primaryRoute)).split('\n\n---\n\n')[1] || "Por favor, describa más detalles sobre su situación actual:"}`;
                ...
                case 'clinica_triage_symptoms': {
                    ...
                    if (isHighSeverity) {
                        ...
                        analyzeClinicalMotive(freeText, telemetry, bodyMapZones).then(aiResult => {
                             ...
                             const reasoningMsg = {
                                 role: "assistant",
                                 content: `**Deducción Clínica Sugerida (Matriz IFM):**\n\n${aiResult.reasoning}`
                             };
                             ...
                             setMessages(prev => [...prev, reasoningMsg, bentoMsg, reviewMsg]);
    ...
    const handleBodyMapComplete = (payload) => {
        ...
        const zoneLabels = {
            ...
            'M_NECK': 'Cuello', 'F_NECK': 'Cuello',
            'M_SHOULDERS': 'Hombros',
            'M_CHEST': 'Pecho', 'F_BREAST_R': 'Seno Derecho', 'F_BREAST_L': 'Seno Izquierdo',
            'M_LUNGS_R': 'Pulmón Derecho', 'M_LUNGS_L': 'Pulmón Izquierdo',
            'F_LUNG_R': 'Pulmón Derecho', 'F_LUNG_L': 'Pulmón Izquierdo',
            'M_STOMACH': 'Estómago', 'F_STOMACH_UP': 'Estómago',
            'M_ABDOMEN_LOW': 'Abdomen Bajo', 'F_STOMACH_LOW': 'Vientre Bajo',
            'F_OVARY_R': 'Ovario Derecho', 'F_OVARY_L': 'Ovario Izquierdo',
            'F_HIPS': 'Caderas',
            'M_LOWER_BACK': 'Espalda Baja', 'F_LOWER_BACK': 'Cintura / Lumbares',
            ...
            'M_ELBOW_R': 'Codo Derecho', 'M_ELBOW_L': 'Codo Izquierdo',
            'M_WRIST_R': 'Muñeca Derecha', 'M_WRIST_L': 'Muñeca Izquierda',
            'M_HAND_R': 'Mano Derecha', 'M_HAND_L': 'Mano Izquierda',
            'F_HAND_R': 'Mano Derecha', 'F_HAND_L': 'Mano Izquierda',
            'M_KNEE_R': 'Rodilla Derecha', 'M_KNEE_L': 'Rodilla Izquierda',
            'F_KNEE_R': 'Rodilla Derecha', 'F_KNEE_L': 'Rodilla Izquierda',
            'M_LEG_R': 'Pierna Derecha', 'M_LEG_L': 'Pierna Izquierda',
            'F_LEG_R': 'Pierna Derecha', 'F_LEG_L': 'Pierna Izquierda',
            'M_ANKLE_R': 'Tobillo Derecho', 'M_ANKLE_L': 'Tobillo Izquierdo',
            'M_FOOT_R': 'Pie Derecho', 'M_FOOT_L': 'Pie Izquierdo',
            'F_FOOT_R': 'Pie Derecho', 'F_FOOT_L': 'Pie Izquierdo'
        };
    ...
    return (
        <>
                ...
                {messages && messages.map((msg, idx) => (
                    ...
                    >
                        ...
                        >
                            ...
                            {/* Opciones de respuesta interactivas (Estilo Fase 1/2 original) */}
                            {msg.role === 'assistant' && idx === messages.length - 1 && messages[messages.length - 1].options?.length > 0 && messages[messages.length - 1].options?.length <= 3 && !isTyping && step !== 'clinica_triage_ai_complete' && (
                                <div className="mt-4 flex flex-wrap gap-2">
                                    {messages[messages.length - 1].options.map((opt, oIdx) => (
                                        <button
                                            key={oIdx}
                                            onClick={(e) => { e.preventDefault(); handleSend(opt.label, opt.value); }}
                                            className="px-4 py-2 bg-white text-slate-700 font-bold rounded-full text-xs hover:bg-slate-50 transition-colors shadow-sm border border-slate-200 disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <div className="p-6 bg-white border-t border-slate-50 shrink-0 relative z-[60]">
                            ...
                            <div className="flex flex-wrap gap-2 justify-center mb-2">
                                {filteredMotiveOptions.map(opt => (
                                    <button 
                                        ...
                                        type="button"
                                        onClick={(e) => { e.preventDefault(); handleSend(opt.label, opt.value); }}
                                        className="px-5 py-2 bg-white text-slate-700 border border-slate-200 hover:bg-slate-50 text-sm font-medium rounded-full transition-colors shadow-sm"
                                    >
                                        {opt.label}
                                    </button>
                                ))}
                ...
                {messages[messages.length - 1]?.options?.length > 3 && !isTyping && (
                ...
                {(!messages[messages.length - 1]?.options || messages[messages.length - 1]?.options?.length === 0) && step !== 'clinica_triage_ai_complete' && step !== 'clinica_triage_containment_complete' && (
                    <form onSubmit={(e) => { e.preventDefault(); if (inputValue.trim() && !isTyping && step !== 'clinica_body_map' && step !== 'clinica_triage_containment' && step !== 'clinica_triage_ai_analysis') handleSend(inputValue); }} className="relative flex items-center gap-2 bg-white border border-slate-200 rounded-full px-2 py-2 shadow-sm focus-within:ring-4 focus-within:ring-blue-50 focus-within:border-blue-400 transition-all w-full z-10">
                        <input
                            type="text"
                            value={inputValue}
