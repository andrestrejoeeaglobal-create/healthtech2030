...
const getFollowUpPrompt = (route, label) => {
    ...
    if (route === 'GOAL_WEIGHT_LOSS') return `Entendido. Hemos activado la ruta de **${label}**.\n\n---\n\n¿Cuáles son sus objetivos principales de composición corporal y qué factores cree que han dificultado su progreso?`;
    if (route === 'GOAL_MUSCLE') return `Entendido. Hemos activado la ruta de **${label}**.\n\n---\n\n¿Cuál es su disciplina deportiva principal, su nivel de entrenamiento actual y sus objetivos de rendimiento o hipertrofia?`;
    if (route === 'GOAL_LONGEVITY') return `Entendido. Hemos activado la ruta de **${label}**.\n\n---\n\n¿Cuáles son los biomarcadores específicos o indicadores de rendimiento cognitivo y físico que busca optimizar en este protocolo?`;
    
    // Grupo C: Transición de Vida (Sin enfoque en patología/dolor)
    if (route === 'GOAL_PREGNANCY') return `Entendido. Hemos activado la ruta de **${label}**.\n\n---\n\n¿En qué etapa gestacional o de lactancia se encuentra actualmente y cuáles son los requerimientos específicos o suplementos que ya le han indicado?`;
    if (route === 'GOAL_GERIATRICS') return `Entendido. Hemos activado la ruta de **${label}**.\n\n---\n\n¿Cuáles son los retos principales en su alimentación diaria, movilidad funcional o masticación que busca mejorar con este abordaje?`;
    if (route === 'GOAL_MENOPAUSE') return `Entendido. Hemos activado la ruta de **${label}**.\n\n---\n\n¿Cuáles son los cambios metabólicos o vasomotores principales que busca modular mediante esta intervención clínica?`;
    if (route === 'GOAL_PEDIATRICS') return `Entendido. Hemos activado la ruta de **${label}**.\n\n---\n\n¿Cuáles son las metas principales de desarrollo, alimentación (ej. inicio de ablactación) o los retos de crecimiento que buscan abordar en esta consulta?`;
...
const Fase3_MotivoConsulta = ({ messages, setMessages, onPhaseComplete, patientData, setPatientData }) => {
    ...
    const getBinaryGateReviewMsg = () => {
        ...
        return {
            role: "assistant",
            content: `📍 **Bloque de Triage Clínico Preliminar Sellado**\n\n*Nota Clínica:* La información capturada y el análisis paramétrico de T.I.L.O. formarán la base integral de su abordaje clínico, en estricto apego a los lineamientos de confidencialidad de la NOM-004.\n\n---\n\n**${displayName}**, por favor valide que esta evaluación clínica es precisa y representa fielmente el motivo de consulta:`,
            options: [
