        ...
        setTimeout(async () => {
            switch (currentPhase) {
                ...
                case 'PHASE_1_PROFILE_LAST_NAME_MAT': {
                    ...
                    setMessages(prev => [...prev, {
                        role: 'assistant',
                        content: `✅ **Perfil biográfico estructurado y blindado** con éxito.\n\n---\n\nUna vez fijada su identidad, el siguiente protocolo es la calibración de su **cronología biológica**, dato indispensable para el cálculo preciso de sus indicadores metabólicos y rangos de referencia clínica.\n\n**${patientData.profile.first_name || 'Paciente'}**, por favor indique únicamente el **DÍA** de su nacimiento (utilice el formato numérico, por ejemplo: 12):`,
                        avatar: tiloImg,
                        inputType: 'number'
                    }]);
                ...
                case 'PHASE_1_PROFILE_LAST_NAME_MAT_MANUAL': {
                    if (text === 'CONFIRM_MAT_NONE') {
                        ...
                        setMessages(prev => [...prev, {
                            role: 'assistant',
                            content: `✅ **Perfil biográfico estructurado y blindado** con éxito (Sin Apellido Materno).\n\n---\n\nUna vez fijada su identidad, el siguiente protocolo es la calibración de su **cronología biológica**, dato indispensable para el cálculo preciso de sus indicadores metabólicos y rangos de referencia clínica.\n\n**${patientData.profile.first_name || 'Paciente'}**, por favor indique únicamente el **DÍA** de su nacimiento (utilice el formato numérico, por ejemplo: 12):`,
                            avatar: tiloImg,
                            inputType: 'number'
                        }]);
                ...
                case 'PHASE_1_PROFILE_DOB_YEAR': {
                    ...
                    let babyMonths = null;
                    if (age === 0) {
                        babyMonths = mDiff >= 0 ? mDiff : (12 + mDiff);
                        if (today.getDate() < checkDate.getDate()) {
                            babyMonths--;
                        }
                        if (babyMonths < 0) babyMonths = 0;
                    }
                    ...
                    let sexMsg = "";
                    if (age < 18) {
                        sexMsg = `Perfil cronológico consolidado.

---

El sistema ha detectado un usuario en etapa de crecimiento activo y ha habilitado el **Protocolo Pediátrico de Nutrición Celular** para optimizar su desarrollo metabólico.
                    ...
                    if (sex === "Masculino" || sex === "Femenino") {
                        ...
                        if (ptCtx && ptCtx.is_minor) {
                            ...
                            if (ptCtx.category === 'ADOLESCENTE') {
                                occupationPrompt = `Protocolo de omisión ejecutado.

---

Al tratarse de un paciente en etapa adolescente, el sistema ha bloqueado automáticamente la recolección de estado civil para proteger el expediente de **${pName}**.

---

Para el registro oficial y cálculo de requerimientos cognitivos: ¿En qué grado o semestre de secundaria o preparatoria se encuentra **${pName}**?`;
                            } else if (ptCtx.category === 'ESCOLAR') {
                                occupationPrompt = `Protocolo de omisión ejecutado.

---

Al tratarse de un paciente en etapa pediátrica, el sistema ha bloqueado automáticamente la recolección de estado civil y actividad laboral para proteger el expediente de **${pName}**.
...
Para cumplir con los lineamientos de seguridad, es obligatorio designar un tutor o contacto de emergencia legal para **${pName}**.
...
Para cumplir con los lineamientos de seguridad, es obligatorio registrar un contacto de emergencia.
...
Por favor, indique el **Nombre Completo** de su contacto responsable:`;
                ...
                case 'PHASE_1_CORRECTION_SELECT': {
                    if (text === 'CANCEL') {
                        ...
                        setMessages([{
                            role: 'assistant',
                            content: "⚠️ **Protocolo de Invalidación Activado**\n\nDirector, si la identidad base (Nombre, CURP, Sexo o Fecha de Nacimiento) es incorrecta en esta etapa, el expediente pierde validez legal.\n\n---\n\nSe ha forzado el cierre del folio para protección de la NOM-004. Por favor, reinicie la toma de datos.",
                            avatar: tiloImg
                        }]);
                        break;
                    } else if (text === 'PHONE') {
                ...
                case 'PHASE_2_EMERGENCY_NAME': {
                    ...
                    if (age < 18) {
                        msgContent = `Entendido.

---

Para establecer el vínculo legal en el expediente, ¿qué parentesco tiene **${eName}** con **${patientFirstName}**?`;
                    } else {
                        msgContent = `Entendido **${patientFirstName}**.

---

Para establecer el vínculo en su expediente, ¿qué parentesco tiene **${eName}** con usted?`;
                    }

                    // Dinámica de Botones (Cruce de Edad + Género)
                    let kinshipOptions = [];
                    const maritalStatus = patientData.profile?.marital_status || patientData.identificacion?.estadoCivil || '';
                    const isSingle = maritalStatus.toLowerCase() === 'soltero' || maritalStatus.toLowerCase() === 'soltera';

                    if (inferredGender === 'FEMALE') {
                        if (age < 12 || (age >= 12 && age < 18)) {
                            kinshipOptions = ['Madre', 'Abuela', 'Tía', 'Hermana Mayor', 'Otra Familiar'];
                        } else {
                            kinshipOptions = isSingle 
                                ? ['Madre', 'Hija', 'Hermana', 'Amiga', 'Otra Familiar']
                                : ['Cónyuge', 'Madre', 'Hija', 'Hermana', 'Amiga', 'Otra Familiar'];
                        }
                    } else {
                        if (age < 12 || (age >= 12 && age < 18)) {
                            ...
                            kinshipOptions = isSingle 
                                ? ['Padre', 'Hijo', 'Hermano', 'Amigo', 'Otro Familiar']
                                : ['Cónyuge', 'Padre', 'Hijo', 'Hermano', 'Amigo', 'Otro Familiar'];
                        }
                    }
