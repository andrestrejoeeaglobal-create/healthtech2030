...
const ChatView = ({ 
    ...
    const [fadeState, setFadeState] = useState('visible'); // visible, fading_out, fading_in
    const [localMultiSelect, setLocalMultiSelect] = useState([]); // Array of selected values

    // Handle Phase Change with Fade Transition (Clear screen effectively by fading out and in)
    useEffect(() => {
        if (currentPhase !== localPhase) {
            setFadeState('fading_out');
            setTimeout(() => {
                setLocalPhase(currentPhase);
                setFadeState('fading_in');
                setTimeout(() => {
    ...
    const isInputDisabled = isTyping || fadeState !== 'visible';
    ...
    return (
        <div className="flex flex-col h-full bg-white relative w-full border-r border-slate-200 shadow-xl z-10">
            {/* MESSAGES AREA */}
            <div className="flex-1 h-full overflow-y-auto p-8 space-y-6 bg-slate-50 custom-scrollbar z-10 relative overflow-x-hidden">
                                                    ...
                                                    <div className="flex flex-col gap-2 mb-3 w-full">
                                                        ...
                                                        <div className="px-4 py-2.5 bg-blue-50/80 backdrop-blur-sm border border-blue-100 rounded-xl text-sm font-semibold text-blue-700 shadow-sm flex items-center gap-3">
                                                            ...
                                                            <div className="flex flex-col">
                                                                ...
                                                                <span>{msg.aiData?.primaryRoute || patientData?.clinical_context?.goal || "Análisis Clínico"}</span>
                                                            </div>
                                                        </div>
                                                        <div className={`px-4 py-2.5 backdrop-blur-sm border rounded-xl text-sm font-semibold shadow-sm flex items-center gap-3 ${
                                                            ...
                                                            <div className="flex flex-col">
                                                                ...
                                                                <span>{msg.aiData?.redFlag ? 'Crítico (Red Flag)' : (msg.aiData?.risk_level === 'HIGH' ? 'Alto' : 'Base')}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                        ...
                                        {msg.role === 'assistant' && msg.showMenu && isLastMessage && (
                                            <div className="mt-4 w-full relative z-50">
                                                 <SearchableVerticalMenu 
                                                    options={msg.showMenuOptions || []} 
                                                    onSelect={(val) => {
                                                        const opt = msg.showMenuOptions.find(o => o.value === val);
