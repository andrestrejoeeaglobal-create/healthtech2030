...
const Fase3_MotivoConsulta = ({ messages, setMessages, onPhaseComplete, patientData, setPatientData, registerInputHandler, setIsGlobalTyping }) => {
...
**${displayName}**, por favor valide que esta evaluación clínica es precisa y representa fielmente el motivo de consulta:`,
    ...
    useEffect(() => {
        if (registerInputHandler) {
            // We pass a wrapped function so it always uses the latest handleSend closure
            registerInputHandler(() => (text, val) => handleSend(text, val));
        }
    }, [registerInputHandler, step, isTyping, inputValue]);
    ...
    // RETURN ONLY CUSTOM INPUT SLOTS (Headless Logic)
    if (step === 'clinica_triage_start' && !isTyping) {
        return (
    ...
    if (step === 'clinica_body_map' && !isTyping) {
        return (
    ...
    // Default: ChatView will render standard text input / options
    return null;
};
